...
for filename in dataset_filenames:
    ...
    print(f"D-Tree für {filename} ist gestartet")
    ...
    # DTree für den Datensatz
    dtree_model = DecisionTree(title=f"DTree für {filename}", dataset=dataset, fast=False)

    dtree_model.tune(dataset, verbose=1)   
    
    # Modell trainieren und bewerten
    train_time = dtree_model.fit(dataset)
    f1_score, run_time, valid_acc, preds = dtree_model.score(dataset, train=False)  # False, um auf Testdaten zu bewerten

    print(f"Trainingszeit: {train_time}s, F1-Score: {f1_score}, Genauigkeit der Vorhersagen: {valid_acc}, Laufzeit: {run_time}s")
    
    if dtree_model.is_trained:
        ...
        # Matthews-Korrelationskoeffizient von -1 (vollständige Unstimmigkeit) bis +1 (perfekte Vorhersage)
        mcc = matthews_corrcoef(dataset.y_test, preds)
        print(f"MCC: {mcc}")
    
    print(f"D-Tree für {filename} ist beendet")
